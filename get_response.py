...
def get_text_from_db():
	...
	for i in range(params.BOT_NUM):
		...
		bot_texts[i][config.QUESTION_INDEX] = Reply(bot_id=i, in_group_id=config.QUESTION_INDEX, texts={modes.GENERAL:questions_at_end}, next_id=config.CLOSING_INDEX)
	...
	bot_texts[0][8] = Reply(bot_id=0, in_group_id=8, texts={modes.GENERAL:[["Cool, looks like you have a plan B.", "Just remember, even though you cannot control everything, there is a way to get back on your feet."]]}, next_id=config.QUESTION_INDEX)
	...
	bot_texts[0][9] = Reply(bot_id=0, in_group_id=9, texts=tmp_text, next_id=config.QUESTION_INDEX)
	del tmp_text
	...
	bot_texts[1][3] = Reply(bot_id=1, in_group_id=3, texts={modes.GENERAL:[["Seems this is not a problem worth worrying about.", " Do you still want to work on it?"], ["In that case, do you feel you\'d still like to spend time working on it?"]]}, next_id=[(config.DEFAULT_NO, config.QUESTION_INDEX), (config.DEFAULT_OTHERS, 4)])
	...
	bot_texts[1][13] = Reply(bot_id=1, in_group_id=13, texts={modes.GENERAL:[["Awesome! When can you do this in the next few days? I find it helpful, to add plans to my calendar."]]}, next_id=config.QUESTION_INDEX)
	...
	bot_texts[1][7] = Reply(bot_id=1, in_group_id=7, texts={modes.GENERAL:[["That\'s alright. Can you "]]}, next_id=config.QUESTION_INDEX)
	...
	bot_texts[1][9] = Reply(bot_id=1, in_group_id=9, texts={modes.GENERAL:[["Great, I\'m glad you can do something about it.", "By breaking down a problem into tiny steps, we can often string together a solution. Hope it helps you."]]}, next_id=config.QUESTION_INDEX)
	...
	# tmp_text[topics.DRIVER] = [["If aggressive driving is a frequent issue, then perhaps taking a different route could solve your problem."]]
	# tmp_text[topics.VEHICLE] = [["Taking your vehicle to the dealership or mechanic is usually a good solution", "as they can pinpoint the exact problem with your vehicle and offer you a solution."]]
	bot_texts[1][10] = Reply(bot_id=1, in_group_id=10, texts=tmp_text, next_id=config.QUESTION_INDEX)
	del tmp_text
	...
	bot_texts[2][5] = Reply(bot_id=2, in_group_id=5, texts={modes.GENERAL:[["Positive thinking can be a good way to destress, making it easier to face challenges."], ["When you\'re feeling down about a situation, trying to find the positives can make it easier to handle."]]}, next_id=config.QUESTION_INDEX)
	...
	bot_texts[3][5] = Reply(bot_id=3, in_group_id=5, texts={modes.GENERAL:[["I\'m glad. Would you consider trying this strategy, such as finding a joke, in the future?"]]}, next_id=config.QUESTION_INDEX)
	bot_texts[3][6] = Reply(bot_id=3, in_group_id=6, texts={modes.GENERAL:[["That\'s ok, humor isn\'t always the answer.", "Just remember that trying to find something funny about your situation can help lighten the mood when you're stressed.", "Sound good?"]]}, next_id=config.QUESTION_INDEX)
	...
	bot_texts[4][13] = Reply(bot_id=4, in_group_id=13, texts={modes.GENERAL:[["Oftentimes, taking a moment to be mindful may help you in situations when you are feeling stressed.", "Sounds good?"]]}, next_id=[(config.DEFAULT_NO, 14), (config.DEFAULT_OTHERS, config.QUESTION_INDEX)])
	bot_texts[4][14] = Reply(bot_id=4, in_group_id=14, texts={modes.GENERAL:[["That\'s ok. There are many other ways to deal with stress."]]}, next_id=config.QUESTION_INDEX)
	#---------------------------------------------------------------------------------------------------------------------------------
	## self-love bot
	...
	bot_texts[5][8] = Reply(bot_id=5, in_group_id=8, texts={modes.GENERAL:[["Awesome, friends can often provide the best support", "It\'s great that you have made time to prioritize yourself!"]]}, next_id=config.QUESTION_INDEX)

	bot_texts[5][4] = Reply(bot_id=5, in_group_id=4, texts={modes.GENERAL:[["I wish I could give you a hug right now to make you feel better!", "I care about you!, and I\'m sure there are others that do too!, even if they are not available right now. ", "I hope I have been able to help!, I\'m here whenever you need me. ", "If you ever want to talk about your problems just find me or one of my friends, ok?"]]}, next_id=config.QUESTION_INDEX)
	...
	# bot_texts[7][7] = Reply(bot_id=7, in_group_id=7, texts={modes.GENERAL:[["Wonderful, it seems as though this problem isn\'t affecting these three major aspects of your life.", "Great job staying on top of your sleeping and eating as well as reaching out to friends and family for support"]]}, next_id=config.QUESTION_INDEX)
	# bot_texts[7][8] = Reply(bot_id=7, in_group_id=8, texts={modes.GENERAL:[[""]]}, next_id=config.QUESTION_INDEX)
	# bot_texts[7][9] = Reply(bot_id=7, in_group_id=9, texts={modes.GENERAL:[[""]]}, next_id=config.QUESTION_INDEX)
	# bot_texts[7][10] = Reply(bot_id=7, in_group_id=10, texts={modes.GENERAL:[[""]]}, next_id=config.QUESTION_INDEX)
	...
	bot_texts[8][1] =  Reply(bot_id=8, in_group_id=1, texts={modes.GENERAL:[["Okay, maybe I can introduce you to my other bot-friends?"]]}, next_id=[(config.DEFAULT_NO, config.ABRUPT_CLOSING_INDEX), (config.DEFAULT_OTHERS, config.QUESTION_INDEX)])
	...
	#{1, 0, 1}
	bot_texts[8][11] = Reply(bot_id=8, in_group_id=11, texts={modes.GENERAL:[["I\'m glad that you have been able to talk with friends and/or family about this.", "Hey! Good job! It seems you\'ve got things mostly under control. It might be helpful to set aside some time for eating"]]}, next_id=config.QUESTION_INDEX)
	
	#{1, 0, 0}
	bot_texts[8][12] = Reply(bot_id=8, in_group_id=12, texts={modes.GENERAL:[["I\'m sorry to hear that. Me and my bot-friends are always here to support you. ","Great job on sleeping regularly! It could be helpful to focus on eating regularly and reaching out to people you care about."]]}, next_id=config.QUESTION_INDEX)
	
	#{0, 1, 1}
	bot_texts[8][13] = Reply(bot_id=8, in_group_id=13, texts={modes.GENERAL:[["I\'m glad that you have been able to talk with friends and/or family about this.","Hey! Good job! It seems you\'ve got things mostly under control. It might be helpful to set aside some time for sleeping."]]}, next_id=config.QUESTION_INDEX)
	
	#{0, 1, 0}
	bot_texts[8][14] = Reply(bot_id=8, in_group_id=14, texts={modes.GENERAL:[["I\'m sorry to hear that. Me and my bot-friends are always here to support you. ","Great job on eating regularly! It might be helpful to focus on sleeping regularly and reaching out to people who care about you."]]}, next_id=config.QUESTION_INDEX)

	#{0, 0, 1}
	bot_texts[8][15] = Reply(bot_id=8, in_group_id=15, texts={modes.GENERAL:[["I\'m glad that you have been able to talk with friends and/or family about this.","Great job on reaching out to people who care about you! It could be helfpul to focus on sleeping regularly and eating regularly."]]}, next_id=config.QUESTION_INDEX)
